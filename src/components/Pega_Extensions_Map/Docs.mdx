...
- If bFreeFormDrawing is true, it is intended to use a location field in the region to render a map component - You can search a location and use the sketch tool
- If bFreeFormDrawing is false, it is intended to draw a list of positions on the map -

The component supports the following features:

- Center on a global map by providing latitude, longitude and zoom
- If bFreeFormDrawing = true, will display the sketch tool to allow for freeform drawing on the map - Note that drawings are temporary and not persisted to the case
